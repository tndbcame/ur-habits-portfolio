...
class FirestoreRepository extends RemoteRepository {
  ...
  @override
  Future<int> existsUser(String userId) async {
    int status = 1;
    try {
  ...
  @override
  Future<int> existsPartner(String userId) async {
    int status = 0;
    try {
      QuerySnapshot docSnapshot = await _firebaseService.firestore
          ...
          .collection('partner')
          .get();
      if (docSnapshot.docs.length >= limitPartner) {
        status = 2;
      } else {
        status = 0;
      }
  ...
  /// パートナーの名前を取得する
  @override
  Future<List<Map<String, dynamic>>> fetchPartnerNames(String userId) async {
  ...
  /// パートナー情報を取得する
  @override
  Future<Map<String, String>> fetchPartnerInfos(String userId) async {
    ...
    try {
      ...
      for (var doc in habitsSnapshot.docs) {
        ...
        List<Map<String, dynamic>> recordsData =
            await _fetchHabitRecords(doc.reference);
  ...
  /// 習慣を取得する(stream)
  @override
  Stream<List<Map<String, dynamic>>> fetchHabitsSnapshot(String userId) async* {
    try {
      List<Map<String, dynamic>> habitsData = [];
      await for (var snapshot in _firebaseService.firestore
          .collection('users')
          .doc(userId)
          .collection('habits')
          .snapshots()) {
  ...
  /// 習慣の記録を取得する
  Future<List<Map<String, dynamic>>> _fetchHabitRecords(
